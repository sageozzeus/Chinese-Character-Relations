...
class _DeckDropdown(QFrame):
    ...
    def __init__(self, parent: Optional[QWidget] = None) -> None:
        super().__init__(parent)
        self.setObjectName("crDeckDropdown")
        self.setCursor(Qt.CursorShape.PointingHandCursor)
...
def _meta_row(form: QFormLayout, label: str, value: str) -> None:
    ...
    # (e.g. "Chinese Character Relations" → only "Chinese Character" visible).
...
class ConfigDialog(QDialog):
    ...
    def _build_display_group(self) -> QGroupBox:
        ...
        self._add_control_cell(caps, 0, 0, "Max per character", self.max_spin)
        ...
        self.include_suspended_toggle.setToolTip(
            "When off, fully suspended notes are hidden from relatives."
        )
    ...
    def _build_index_group(self) -> QGroupBox:
        ...
        tip = QLabel(
            ...
            "in Relatives. Appearance changes do not need a rebuild."
        )
        tip.setWordWrap(True)
        tip.setStyleSheet("opacity: 0.7; font-size: 11px;")
        layout.addWidget(tip)
        ...
        self.rebuild_btn.setToolTip("Scan decks and refresh the character → notes index.")
    ...
    def _build_type_group(self) -> QGroupBox:
        ...
        self._add_control_cell(grid, 0, 0, "Character (em)", self.char_size_spin)
        self._add_control_cell(grid, 0, 1, "Related word (em)", self.word_size_spin)
